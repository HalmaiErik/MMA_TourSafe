...
@Component("Weekly")
public class WeeklyScheduler implements ScheduleStrategy {
    private final MatchRepository matchRepository;

    @Autowired
    public WeeklyScheduler(MatchRepository matchRepository) {
    ...
    private LocalDateTime findNewMatchTime(TournamentDTO tournamentDTO) {
        ...
        if (scheduledMatches.isEmpty()) {
            ...
            if (tournamentEndDate.isAfter(lastMatchTime.plusWeeks(1).toLocalDate())) {
                return lastMatchTime.plusWeeks(1);
            }
            else {
